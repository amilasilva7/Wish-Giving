...
const WISHES: Array<{ title: string; description: string; category: string; occasionType: string }> = [
  {
...
async function main() {
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    // Upsert user so the script is safe to re-run
    ...
    // Create 3 wishes for this user (skip if they already have wishes)
    ...
    console.log(`✓ ${email} — 3 wishes seeded`);
  }

  console.log("\nDone. All 10 test users ready.");
